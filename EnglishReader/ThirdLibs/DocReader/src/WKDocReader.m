...
-(void)fillStringWithDefaultAttributes
{
	...
	[self setAttribute:kCTFontAttributeName toValue:(__bridge id)defaultFont startingAtIndex:0 endingAtIndex:[self length]];
...
-(void)setAttribute:(CFStringRef)attribute toValue:(id)value startingAtIndex:(NSUInteger)index endingAtIndex:(NSUInteger)endIndex;
{
	...
	if([existingAttributes objectForKey:(__bridge NSString *)attribute]) {
		[existingAttributes removeObjectForKey:(__bridge NSString *)attribute];
	}
	[existingAttributes setObject:value forKey:(__bridge NSString *)attribute];
	...
	while(![fmtStream isAtEnd]) {
		...
		switch (sprm) {
			...
			case sprmCHighlight: {
				...
				[_attributedString setAttribute:(CFStringRef)WKBackgroundColorAttributeName toValue:(__bridge id)color startingAtIndex:start endingAtIndex:index];
				break;
			}
				
			case sprmCFBold: {
				BOOL shouldBeBold = ([fmtStream readUInt8] == 0x01);
				CTFontRef existingFont = (__bridge CTFontRef)[[_attributedString attributesAtIndex:start effectiveRange:NULL] objectForKey:(id)kCTFontAttributeName];
				...
				[_attributedString setAttribute:kCTFontAttributeName toValue:(__bridge id)newFont startingAtIndex:start endingAtIndex:index];
			...
			case sprmCIco: {
				...
				[_attributedString setAttribute:kCTForegroundColorAttributeName toValue:(__bridge id)textColor startingAtIndex:start endingAtIndex:index];
				break;
			}
			...
			case sprmCHps: {
				...
				CTFontRef existingFont = (__bridge _Nullable CTFontRef)[[_attributedString attributesAtIndex:start effectiveRange:NULL] objectForKey:(id)kCTFontAttributeName];
			...
			//	CTFontSymbolicTraits traits = CTFontGetSymbolicTraits(existingFont);
				...
				[_attributedString setAttribute:kCTFontAttributeName toValue:(__bridge id)newFontSansTraits startingAtIndex:start endingAtIndex:index];
			...
			case sprmCCvUl: {
				...
				[_attributedString setAttribute:kCTUnderlineColorAttributeName toValue:(__bridge id)underlineColor startingAtIndex:start endingAtIndex:index];
			}
...
-(void)readStylesheet
{
	...
	[_attributedString setAttribute:kCTFontAttributeName toValue:(__bridge id)defaultFont startingAtIndex:0 endingAtIndex:[_attributedString length]];
	CFRelease(defaultFont);
...
-(CTParagraphStyleRef)createModifiedParagraphStyle:(CTParagraphStyleRef)originalStyle specifier:(CTParagraphStyleSpecifier)specifier newValue:(void *)newValuePtr
{
	...
	for(int i = 0; i < kCTParagraphStyleSpecifierCount; i++) {
        ...
        size_t bufferSize = paragraphStyleSpecifierSizes[i];
		void *oldValuePtr =  malloc(bufferSize);
		CTParagraphStyleGetValueForSpecifier(originalStyle, i, bufferSize, oldValuePtr);
...
-(void)setFailureReason:(NSString *)reason
{
	...
	_error = [NSError errorWithDomain:NSCocoaErrorDomain code:0 userInfo:[NSDictionary dictionaryWithObject:reason forKey:NSLocalizedFailureReasonErrorKey]];
