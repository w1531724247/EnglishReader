function wrapTextToNode(aNode) {
    ...
    while (hasSubTags(currentNode)) {//等于1表示没有子节点, 但有一个隐式的文本节点, 文本也是一个节点
        ...
        for (var index = 0; index < currentNode.childNodes.length; index++) {
            ...
            if (subNode.tagName != undefined) {//有标签名
                var wrapedFlag = subNode.getAttribute("wraped");
                if (wrapedFlag != "true") {//并且没有被包裹
            ...
            if (index == currentNode.childNodes.length - 1) {//是最后一个
                iteratorOver = true;
            }
        }
    ...
    console.log("currentNode" + currentNode.tagName + ": " + currentNode.innerText + "__length: " + currentNode.childNodes.length);
    for (var subNode in currentNode.childNodes) {
        console.log("subNode" + subNode.tagName + ": " + subNode.innerText + "__class: " + subNode.className);
    }
    debugger;
        ...
        for (var index = 0; index < brotherNodes.length; index++) {
            ...
            if (previonsText.length > 0) {
                ...
                if (isExceptNode(node)) {
                    clone_parentNode.appendChild(node.cloneNode(true));
                } else {
